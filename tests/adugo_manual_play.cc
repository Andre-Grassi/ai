...
// Parse action string in server format: "<type> <from_row> <from_col> <to_row>
// <to_col>" type: 'm' for move, 's' for capture sequence Example: "m 5 2 5 3"
// or "s 5 2 3 2"
adugo_game::Action ParseActionString(const std::string& action_str,
                                     adugo_game::Symbol player_symbol) {
    char move_type;
    int from_row, from_col, to_row, to_col;

    // Parse the action string
    int parsed = std::sscanf(action_str.c_str(), "%c %d %d %d %d", &move_type,
                             &from_row, &from_col, &to_row, &to_col);

    if (parsed != 5) {
        throw std::invalid_argument(
            "Invalid action format. Expected: <type> <from_row> <from_col> "
            "<to_row> <to_col>");
    }

    if (move_type != 'm' && move_type != 's') {
        throw std::invalid_argument(
            "Invalid move type. Must be 'm' (move) or 's' (capture sequence)");
    }

    // Convert positions to indices
    int from_index = PositionToIndex(from_row, from_col);
    int to_index = PositionToIndex(to_row, to_col);

    return adugo_game::Action(player_symbol, from_index, to_index);
}

std::string GetUserAction() {
    std::string action_str;
    std::cout << "\nEnter action (format: <type> <from_row> <from_col> "
                 "<to_row> <to_col>): ";
    std::cout.flush();
...
int main(int argc, char** argv) {
    ...
    constexpr int kMaxDepth = 15;  // A safe bet for the depth regarding time
    ...
    while (true) {
        ...
        if (current_state.player_to_move.symbol == my_player.symbol) {
            ...
            std::string action_input = GetUserAction();

            std::unique_ptr<Action> best_action;
            try {
                Action parsed_action =
                    ParseActionString(action_input, my_player.symbol);
                best_action = std::make_unique<Action>(parsed_action);
